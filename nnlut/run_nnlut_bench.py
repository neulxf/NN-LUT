...
from nnlut import OneHiddenReLU, export_lut, lut_forward_fast   
################################################################################
# 1. 准备 FP16 值全集（≈1e5 个）
################################################################################
...
def train_and_eval(op_key: str, hidden: int, full_fp16=False,
    ...
    mask_vis = (xs_plot >= x_lo) & (xs_plot <= x_hi)
    xs_vis   = xs_plot[mask_vis]
    ys_true_vis = ys_true[mask_vis]
    ys_pred_vis = ys_pred[mask_vis]
    abs_err_vis = abs_err_np[mask_vis]
    ...
    ax_top.plot(xs_vis, ys_true_vis, label="ground‑truth", color="tab:blue")
    ...
    for i in range(len(slopes)):
        ...
        ax_top.plot(
            ...
            color="tab:orange",
            linewidth=1.3,
            label="NN‑LUT approx" if i == 0 else None,  # 仅首段加 label
        )
    ...
    plot_fname = f"{save_dir}/lut_plot_{op_key}_H{hidden}_{'full' if full_fp16 else 'sub'}.png"
