...
from transformers.models.qwen3_moe.modeling_qwen3_moe import Qwen3MoeDecoderLayer, Qwen3MoeMLP, Qwen3MoeSparseMoeBlock
...
def main(args): 
    ...
    no_split_module_classes = ['LlamaDecoderLayer','QuantDecoderLayer',"Qwen2DecoderLayer","Qwen3DecoderLayer", "Qwen3MoeDecoderLayer"]
    if args.use_fplut:
        for layer in model.model.layers:
            if isinstance(layer.mlp, Qwen3MoeSparseMoeBlock):
                for expert in layer.mlp.experts:
                    expert.act_fn = FPLUT(func_name="silu", function=expert.act_fn)
            else:
                layer.mlp.act_fn = FPLUT(func_name="silu", function=layer.mlp.act_fn)
                layer.self_attn.fp_softmax = FPSoftMax()
                layer.self_attn.forward = types.MethodType(qwen3_attn_forward, layer.self_attn)
    elif args.use_nnlut:
        for layer in model.model.layers:
            ...
            if isinstance(layer.mlp, Qwen3MoeSparseMoeBlock):
                for expert in layer.mlp.experts:
                    expert.act_fn = Nnlut(lut_details["cut_points"], lut_details["slopes"], lut_details["biases"])
            else:
    ...
    if args.calc_range:
        ...
        for lyr in model.model.layers:
            # --- SiLU / activation ---
            if isinstance(lyr.mlp, Qwen3MoeSparseMoeBlock):
                for expert in lyr.mlp.experts:
                    if isinstance(expert.act_fn, FPLUT):
                        expert.act_fn.register_forward_hook(_silu_hook)
            else:
