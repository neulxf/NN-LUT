...
def test_op(op_key, hidden, save_dir):
    cfg   = OP_CFG[op_key]
    func  = cfg["func"]
    ...
    for i in range(len(table.cut_points)):
        ...
        ax_top.plot(
            ...
            color="tab:green",
            linewidth=1.3,
            label="NLI approx" if i == 0 else None,  # 仅首段加 label
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_dir", type=str, default="NLI_lut_plot")
    args = parser.parse_args()
    for op_key in OP_CFG.keys():
        print(f"Testing {op_key}...")
        test_op(op_key, 11, args.save_dir)